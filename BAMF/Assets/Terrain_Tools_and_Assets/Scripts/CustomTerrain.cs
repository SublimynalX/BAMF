...
//Allows code to run while in edit mode
[ExecuteInEditMode]
public class CustomTerrain : MonoBehaviour
{

    //holds min and max height randomly
    public Vector2 randomHeightRange = new Vector2(0, 0.1f);
    //holds image
    public Texture2D heightMapImage;
    //sets scaling value for image
    ...
    //grab hold of terrain object
    public Terrain terrain;
    //holds all dat for terrain
    ...
    public void GenerateClouds()
    {
        ...
        for(int c = 0; c < numClouds; c++)
        {
            ...
            Vector3 newScale = new Vector3(UnityEngine.Random.Range(cloudScaleMin.x, cloudScaleMax.x), UnityEngine.Random.Range(cloudScaleMin.y, cloudScaleMax.y),
                ...
                foreach(Vector2 n in neighbors)
                {
                    if(heightMap[x,y] < waterHeight && heightMap[(int)n.x, (int)n.y] > waterHeight)
                    {
                        ...
                        //{
                            ...
                            waterHeight * terrainData.size.y, x/ (float)terrainData.heightmapWidth * terrainData.size.x);


                        go.transform.LookAt(new Vector3(n.y / (float)terrainData.heightmapHeight * terrainData.size.z, waterHeight * terrainData.size.y,
            ...
            for(int x = 0; x < terrainData.size.x; x += treeSpacing)
            {
                for(int tp = 0; tp < terrainData.treePrototypes.Length; tp++)
                {
                    ...
                    if ((thisHeight >= thisHeightStart && thisHeight <= thisHeightEnd)&& (steepness >= vegetation[tp].minSlope&& steepness <= vegetation[tp].maxSlope))
                    {
                        ...
                        terrainData.GetHeight(x, z) / terrainData.size.y, (z + UnityEngine.Random.Range(-5.0f, 5.0f)) / terrainData.size.z);

                        Vector3 treeWorldPos = new Vector3(instance.position.x * terrainData.size.x,
                        instance.position.y * terrainData.size.y, instance.position.z * terrainData.size.z) + this.transform.position;
    ...
    void DigCanyon()
    {
        ...
        {
            CanyonCrawler(cx, cy, tempHeightMap[cx, cy] - digDepth, bankSlop, maxDepth);
    ...
    void Tidal()
    {
        ...
        for(int y = 0; y < terrainData.heightmapHeight; y++)
        {
            ...
            {
                Vector2 thisLocation = new Vector2(x, y);
                List<Vector2> neighbors = GenerateNeighbors(thisLocation, terrainData.heightmapWidth,
                terrainData.heightmapHeight);
    ...
    void River()
    {
        ...
        for(int i = 0; i< droplets; i++)
        {
            ...
            {
                erosionMap = RunRiver(dropletPosition, heightMap, erosionMap, terrainData.heightmapWidth, terrainData.
                heightmapHeight);
            }
        }
        for(int y = 0; y < terrainData.heightmapHeight; y++)
    ...
    {
        //Creates first empty list so GUITAble doesnt complain
    ...
    public void RemoveSplatHeights()
    {
        ...
        if(keptSplatHeights.Count == 0)//dont wanna keep any
        {
            keptSplatHeights.Add(splatHeights[0]);//got to have at least one
        }
    ...
    float GetSteepness(float[,]heightmap,int x, int y, int width, int height)
    {
        ...
        //if on upper edge, will find gradient by goin backwards
    ...
    public void Voronoi()
    {
        ...
        for (int p = 0; p < voronoiPeaks; p++)
        {
            ...
            //got to peak x and z index and set it to y value
            ...
            //Set the distance to a slope between the peak and other points in the terrain

            Vector2 peakLoacation = new Vector2(peak.x, peak.z);
            ...
            for (int y = 0; y < terrainData.heightmapHeight; y++)
            {
                for (int x = 0; x < terrainData.heightmapWidth; x++)
                {
                    ...
                    {
                        float distanceToPeak = Vector2.Distance(peakLoacation, new Vector2(x, y)) / maxDistance;
                        float h;
    ...
    public void MidPointDisplacement()
    {
        ...
        heightMap[terrainData.heightmapWidth - 2, terrainData.heightmapHeight - 1] = UnityEngine.Random.Range(0f, 0.2f);*/
        while (squareSize > 0)
        {
            ...
            for(int x = 0; x < width; x+=squareSize)
            {
                for(int y =0;y < width; y+=squareSize)
                {
                    ...
                    heightMap[midX, y] = (float)((heightMap[midX, midY] + heightMap[x, y] + heightMap[midX, pmidYD] + heightMap[cornerX, y]) 
                    / 4.0f + UnityEngine.Random.Range(heightMin, heightMAx));

                    //Top Side
                    heightMap[midX, cornerY] = (float)((heightMap[x, cornerY] + heightMap[midX, midY] + heightMap[cornerX, cornerY] + heightMap[midX, pmidYU])
                    / 4.0f + UnityEngine.Random.Range(heightMin, heightMAx));

                    //Left Side
                    heightMap[x, midY] = (float)((heightMap[x, y] + heightMap[pmidXL, midY] + heightMap[x, cornerY] + heightMap[midX, midY])
                    / 4.0f + UnityEngine.Random.Range(heightMin, heightMAx));

                    //Right Side
                    heightMap[cornerX, midY] = (float)((heightMap[midX, y] + heightMap[midX, midY] + heightMap[cornerX, cornerY] + heightMap[pmidXR, midY])
                    / 4.0f + UnityEngine.Random.Range(heightMin, heightMAx));
                }
            }
    ...
    {
        List<Vector2> neightbors = new List<Vector2>();
        for(int y = -1; y < 2; y++)
        {
            for(int x = -1; x < 2; x++)
            {
    ...
    //Where all processing for height values takes place
    public void RandomTerrain()
    {
        //heightvalue setup, new heightmap
        float[,] heightMap = GetHeightMap();
    ...
    public void LoadTexture()
    {
        ...
        for(int x = 0; x < terrainData.heightmapWidth; x++)
        {
            for(int z = 0; z< terrainData.heightmapHeight; z++)
            {
                //sets value as color from texture
                //grayscale converts the color to black and white
    ...
    private void OnEnable()
    {
        ...
        //set terrain data and store it
        terrainData = Terrain.activeTerrain.terrainData;

    }
    //creates the tags needed for the editor 
    // all the serialized stuff links objects to the editor
